                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" 
                                   ...
                                   id="card_code" 
                                   name="card_code" 
                                   value="{{ old('card_code', $card->card_code) }}" 
                            ...
                            <select class="form-control @error('racer_id') is-invalid @enderror" 
                                ...
                                @foreach($racers as $racer)
                                    <option value="{{ $racer->id }}" 
                                            {{ old('racer_id', $card->racer_id) == $racer->id ? 'selected' : '' }}>
                                        {{ $racer->racer_name }} 
                                        @if($racer->team)
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('status') is-invalid @enderror" 
                                ...
                                <option value="BANNED" {{ old('status', $card->status) == 'BANNED' ? 'selected' : '' }}>BANNED</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
